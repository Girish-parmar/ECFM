...
def validate(name: str, bars, signal_fn, grid: dict, log, train: int = 750, test: int = 250) -> dict:
    ...
    # >>> SOLUTION
    run = make_runner(bars, signal_fn)
    keys = list(grid)
    cols = []
    for combo in itertools.product(*grid.values()):
        ...
        mdd = float(np.min(np.cumprod(1 + pnl) / np.maximum.accumulate(np.cumprod(1 + pnl)) - 1))
        log.record(name, p, "synthetic-v1", float(sr), mdd, int(pnl.size))
        cols.append(pnl)
    matrix = np.column_stack(cols)
    wf = op.walk_forward_optimize(lambda **p: run(p), grid, len(bars), train, test)
    pbo = ov.pbo_cscv(matrix, S=10)[0]
    trial_srs = log.trials(name)["sharpe"].to_numpy() / np.sqrt(252)
    card = ov.robustness_scorecard(run, wf["params"][-1])
    gate = ov.validation_gate(wf["oos"], trial_srs, pbo, card)
    return {"gate": gate, "scorecard": card, "pbo": pbo, "wf": wf, "dossier": ov.dossier_markdown(name, gate, card)}
    # <<< SOLUTION
