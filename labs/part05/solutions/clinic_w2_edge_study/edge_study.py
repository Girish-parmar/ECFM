...
def edge_table(universe: dict[str, pd.DataFrame], patterns: dict, horizons=(5,), n_perm: int = 1000,
    ...
    for sym, df in universe.items():
        o, h, l, c, _ = arrays(df)  # noqa: E741
        for name, fn in patterns.items():
            sig = np.asarray(fn(o, h, l, c))
            for side, val in (("bull", 1), ("bear", -1)):
                ev = sig == val
                if not ev.any():
                    continue
                for hz in horizons:
                    r = gr.pattern_edge(ev, o, horizon=hz, n_perm=n_perm, seed=0)
                    if r["n"]:
                        rows.append({"pattern": name, "side": side, "symbol": sym, "horizon": hz, **r})
    out = pd.DataFrame(rows)
    out["q_value"] = gr.bh_adjust(out["p_value"].to_numpy())
    out["discovery"] = out["q_value"] <= alpha
    return out
    # <<< SOLUTION
...
def summary(table: pd.DataFrame) -> pd.DataFrame:
    ...
    # >>> SOLUTION
    g = table.assign(w=table["edge"] * table["n"]).groupby(["pattern", "side"])
    return pd.DataFrame({"tests": g.size(), "events": g["n"].sum(), "mean_edge": g["w"].sum() / g["n"].sum(),
                         "share_p05": g["p_value"].apply(lambda p: (p < 0.05).mean()),
                         "discoveries": g["discovery"].sum()}).reset_index().sort_values(["pattern", "side"],
                                                                                       ignore_index=True)
    # <<< SOLUTION
