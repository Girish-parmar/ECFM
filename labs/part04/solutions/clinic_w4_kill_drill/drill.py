...
async def load_book(broker, n_positions: int, n_orders: int) -> None:
    ...
    for i in range(n_positions):
        ...
        side, qty = (Side.BUY, "10") if i % 2 == 0 else (Side.SELL, "5")
        await broker.submit(OrderRequest(Instrument(sym, AssetClass.EQUITY), side, Decimal(qty), OrdType.MARKET))
    px = broker.last[SYMBOLS[0]] * Decimal("0.9")
    for i in range(n_orders):
        await broker.submit(OrderRequest(Instrument(SYMBOLS[0], AssetClass.EQUITY), Side.BUY, Decimal("1"),
                                         OrdType.LIMIT, limit_price=(px - i).quantize(Decimal("0.01"))))
    # <<< SOLUTION
...
async def run_drill(state_dir: Path, latency: float = 0.2, n_positions: int = 3, n_orders: int = 5,
    ...
    brokers = [SlowBroker("ib-sim", latency, prices), SlowBroker("alpaca-sim", latency, prices)]
    for b in brokers:
        await load_book(b, n_positions, n_orders)
    ks = sf.KillSwitch(brokers, Path(state_dir) / "killswitch.json")
    t0 = time.perf_counter()
    await ks.trip("kill-switch drill")
    elapsed = time.perf_counter() - t0
    flat = all(b.is_flat() for b in brokers)
    return {"elapsed": elapsed, "flat": flat, "open_orders": sum(len(b.open) for b in brokers),
            "tripped": ks.tripped, "passed": flat and elapsed < limit}
    # <<< SOLUTION
...
async def watch(dog: Watchdog, switch, poll: float = 1.0, max_polls: int | None = None,
    ...
    while max_polls is None or polls < max_polls:
        if dog.stale:
            await switch.trip("heartbeat lost")
            return True
        polls += 1
        await sleep(poll)
    return False
