...
def build_index(filings: list[dict], drop_sections: tuple[str, ...] = ()) -> tuple[list, object]:
    ...
    chunks = [c for f in filings for c in nr.chunk_filing(f["text"], f["ticker"], f["form"], f["filed_at"])
              if c.section not in drop_sections]
    return chunks, nr.Retriever(chunks)
    # <<< SOLUTION
...
def retrieval_table(retriever, chunks, questions: list[dict], k: int = 5) -> pd.DataFrame:
    ...
    for mode in ("bm25", "dense", "hybrid"):
        got = [retriever.search(q["question"], q["ticker"], q["as_of"], k, mode) for q in qs]
        rows[mode] = {"recall@1": nr.recall_at_k(got, rel, 1), f"recall@{k}": nr.recall_at_k(got, rel, k),
                      "mrr": nr.mrr(got, rel)}
...
def answer(retriever, chunks, q: dict, sanitize: bool = True, obedient: bool = False) -> dict:
    ...
    flagged = []
    if sanitize:
        sent, flagged = nr.sanitize(sent)
    req = nr.citation_request(q["question"], sent)
    parsed = nr.parse_cited_answer(offline_answer(q["question"], req["messages"][0]["content"][:-1], obedient))
    return {"text": " ".join(t for t, _ in parsed), "parsed": parsed,
            "docs": {nr.doc_title(c): c.text for c in sent}, "flagged": len(flagged)}
    # <<< SOLUTION
...
def answer_table(retriever, chunks, questions: list[dict], facts: pd.DataFrame, **kw) -> pd.DataFrame:
    ...
    rows = []
    for q in questions:
        a = answer(retriever, chunks, q, **kw)
        refused = a["text"].startswith("Not found")
        row = {"kind": q["kind"], "faithful": nr.faithfulness(a["parsed"], a["docs"]),
               "refusal_ok": refused == (q["kind"] == "not_found"), "numeric_ok": np.nan}
        if q["kind"] == "numeric":
            year = int(q["question"].split("fiscal ")[1][:4])
            truth = facts.loc[(facts["ticker"] == q["ticker"]) & (facts["fiscal_year"] == year), "revenue_usd_m"]
            got = nr.first_amount(a["text"])
            row["numeric_ok"] = got is not None and abs(got - float(truth.iloc[0])) < 0.05
        rows.append(row)
...
def adversarial_pass_rate(retriever, chunks, questions: list[dict], facts: pd.DataFrame, sanitize: bool) -> float:
    ...
    refused."""
    # >>> SOLUTION
    adv = [q for q in questions if q["kind"] == "not_found" or (q["ticker"] == "EPIC" and "2024" in q["question"])]
    t = answer_table(retriever, chunks, adv, facts, sanitize=sanitize, obedient=True)
    ok = np.where(t["kind"] == "numeric", t["numeric_ok"].fillna(False).astype(bool), t["refusal_ok"])
