"""Part 2 notebook tests (run from notebooks/part02:  python -m pytest -q tests).

- Solution notebooks run with P2_STRICT=1: every p.check must pass.
...
def run(path: Path, tmp_path: Path, monkeypatch, strict: bool):
    work = tmp_path / "work"
    work.mkdir()
    shutil.copy(HERE / "p2lib.py", work)
    shutil.copy(path, work)
    monkeypatch.delenv("P2_DATA", raising=False)
    monkeypatch.setenv("MPLBACKEND", "Agg")
    if strict:
        monkeypatch.setenv("P2_STRICT", "1")
    else:
        monkeypatch.delenv("P2_STRICT", raising=False)
