"""Part 7 notebook tests (run from notebooks/part07:  python -m pytest -q tests).

- Solution notebooks run with P7_STRICT=1: every p.check must pass.
...
def run(path: Path, tmp_path: Path, monkeypatch, strict: bool):
    work = tmp_path / "work"
    work.mkdir()
    shutil.copy(HERE / "p7lib.py", work)
    shutil.copy(path, work)
    monkeypatch.setenv("MPLBACKEND", "Agg")
    if strict:
        monkeypatch.setenv("P7_STRICT", "1")
    else:
        monkeypatch.delenv("P7_STRICT", raising=False)
